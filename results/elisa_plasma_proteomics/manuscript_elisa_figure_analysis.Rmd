...
get_ratio_data <- function(data_long2){
  ## Data wrangling ----
...
<!-- # Data File Paths -->
```{r message=FALSE, warning=FALSE, include=FALSE}
patient_clinical_data <- here::here("data/elisa_plasma_proteomics/Patients_information_FINAL.csv")

patient_longitudinal_mapping_file <- here::here("data/elisa_plasma_proteomics/Patients_for_ELISA_longitudinal_mapping.csv")

elisa_data_file <- here::here("data/elisa_plasma_proteomics/ELISA_data.csv")
```
...
patient_id_meta <- unique(data_long[, .(LAB_ID, Number.x, collection_type, manifestation_type, patient_state, `Time-to-reccurence (days)`)])
```
...
MyResult.splsda <- mixOmics::splsda(train_df[, c(-1, -2, -7, -8, -9, -10)], train_df$class, keepX = c(4,4)) 
MyResult.splsda <- mixOmics::splsda(train_df[, c(-1)], train_df$class, keepX = c(4,4))
background <- background.predict(MyResult.splsda, comp.predicted=2, dist = "max.dist") 

plsda_data <- cbind( patient_id=na.omit(data_wide %>% dplyr::filter(Number.x!=32))  %>% dplyr::pull(Number.x), class=train_df$class, as.data.frame(MyResult.splsda$variates$X))
plsda_data$patient_id <- as.factor(plsda_data$patient_id)
plsda_data %>%
  dplyr::arrange(class) -> plsda_data
plsda_data <- as.data.table(plsda_data)
...
p <- ggplot() +
  ...
  guides(shape="none",
         ...
         colour_new=guide_legend(title="Class", ncol=1, byrow = T),
         col=guide_legend(title="Class", ncol=1, byrow = T))
  # guides(shape="none",
  #        colour_new_new=guide_legend(title="Class", ncol=1, byrow = T),
  #        colour_new='none',
  #        col=guide_legend(title="Trajectory", ncol=1))
...
background %>%
  dplyr::filter(Var2 < 1.59058 & Var2 > -1.56044 & Var1 > -2.5462 & Var1 < 2.36565) -> background_line

lm_eq <- lm(Var2 ~ Var1, data=background_line)
coef(lm_eq)

plsda_data %>%
  dplyr::rowwise() %>%
  dplyr::mutate(new_x = (comp1 + -1/coef(lm_eq)[2]*comp2  - (-1/coef(lm_eq)[2]*(-coef(lm_eq)[1])))/(1+(-1/coef(lm_eq)[2])^2),
                new_y = (-1/coef(lm_eq)[2]*comp1 + ((-1/coef(lm_eq)[2])^2)*comp2  + (-coef(lm_eq)[1]))/(1+(-1/coef(lm_eq)[2])^2)) -> plsda_data_proj

ggplot() +
  # geom_point(data=background_line, aes(x=Var1, y=Var2)) +
  geom_abline(slope = coef(lm_eq)[2], intercept = coef(lm_eq)[1]) +
  geom_abline(slope = -1/coef(lm_eq)[2], intercept = -coef(lm_eq)[1]) +
  geom_point(data=plsda_data_proj, aes(x=new_x, y=new_y, col=class)) 
...
ggplot() +
  ...
  # geom_point(data=plsda_data[patient_id=="Ctrl"], aes(x=comp1, y=comp2, shape=class), col="black", size=3) +
  # geom_point(data=plsda_data[patient_id!="Ctrl"], aes(x=comp1, y=comp2, col=class, shape=class), size=3) +
  geom_point(data=plsda_data, aes(x=comp1, y=comp2, col=class, shape=class), size=3, alpha=0.45) +
  scale_shape_manual(values=c(15, 16, 17)) +
  scale_color_manual(values=c( "#388ECC", "#F68B33"), labels=c(expression(paste(frac('initial GBM', 'recurrent GBM'), " Ratio")),
                                                               expression(paste(frac('recurrent GBM', 'initial GBM'), " Ratio")))
  ) +
  # ggnewscale::new_scale_color() +
  # ggrepel::geom_label_repel(data=merge(plsda_data %>% dplyr::mutate(patient_id = as.numeric(as.character(patient_id))), unique(data_long[manifestation_type!='' & patient_state==1 & `Time-to-reccurence (days)`!='', .(Number.x, `Time-to-reccurence (days)`)]), by.x='patient_id', by.y="Number.x", all.x=T), aes(x=comp1, y=comp2, col=`Time-to-reccurence (days)`, label=`Time-to-reccurence (days)`)) +
  ggnewscale::new_scale_color() +
  ...
  #                         length=unit(0.1, "inches")), size=2, alpha=0.8) +
  geom_abline(slope = -1/coef(lm_eq)[2], intercept = -coef(lm_eq)[1]) +
  geom_point(data=plsda_data_proj, aes(x=new_x, y=new_y, col=class, shape=class), size=3)  +
  scale_shape_manual(values=c(15, 16, 17)) +
  scale_color_manual(values=c( "#388ECC", "#F68B33"), labels=c(expression(paste(frac('initial GBM', 'recurrent GBM'), " Ratio")),
                                                               expression(paste(frac('recurrent GBM', 'initial GBM'), " Ratio")))
  ) +
  ...
  guides(shape="none",
         shape_new="none",
         colour_new="none",
         col=guide_legend(title="Class", ncol=1))

ggplot() +
  geom_point(data=merge(plsda_data_proj %>% dplyr::filter(class=='rGBM') %>% dplyr::mutate(patient_id = as.numeric(as.character(patient_id))), unique(data_long[manifestation_type!='' & patient_state==1 & `Time-to-reccurence (days)`!='', .(Number.x, `Time-to-reccurence (days)`)]), by.x='patient_id', by.y="Number.x", all.x=T), aes(x=class, y=-new_x, col=`Time-to-reccurence (days)`)) +
  ggrepel::geom_label_repel(data=merge(plsda_data_proj %>% dplyr::filter(class=='rGBM') %>% dplyr::mutate(patient_id = as.numeric(as.character(patient_id))), unique(data_long[manifestation_type!='' & patient_state==1 & `Time-to-reccurence (days)`!='', .(Number.x, `Time-to-reccurence (days)`)]), by.x='patient_id', by.y="Number.x", all.x=T), aes(x=class, y=-new_x, col=`Time-to-reccurence (days)`, label=`Time-to-reccurence (days)`)) +
  labs(y="Projected X-variate 1", x=NULL) +
  scale_x_discrete(labels=c(expression(paste(frac('recurrent GBM', 'initial GBM'), " Ratio")))) +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5, size=17),
        axis.title = element_text(size = 17),
        axis.text = element_text(size=16),
...
## Data wrangling ----
# Get ratio data
message("Get Ratio Data")
ratio_data <- get_ratio_data(data_long2)
message("Get Test Train Data")
train_test <- get_train_test_for_loocv(ratio_data, test_patient_id=NULL, unique_random_train_patient_per_class=T)
train_df <- train_test$train_df
test_df <- train_test$test_df
test_patients <- test_df$Number.x
train_df <- rbindlist(list(train_df, test_df))

## sPLS-DA of Ratios ----
message("Perform sPLS-DA")
MyResult.splsda <- mixOmics::splsda(train_df[, c(-1, -2, -7, -8, -9, -10)], train_df$class, keepX = c(4,4)) 
MyResult.splsda <- mixOmics::splsda(train_df[, c(-1, -2)], train_df$class, keepX = c(4,4)) 

pred_test <- predict(MyResult.splsda, test_df[, c(-1, -2, -7, -8, -9, -10)], test_df$class)
pred_test <- predict(MyResult.splsda, test_df[, c(-1, -2)], test_df$class)
message("Perform background predictions")
...
background <- background.predict(MyResult.splsda, comp.predicted=2, dist = "max.dist", xlim = c(min(c(MyResult.splsda$variates$X)), max(c(MyResult.splsda$variates$X))), ylim = c(min(c(MyResult.splsda$variates$X)), max(c(MyResult.splsda$variates$X))))
message("Prepare data for plotting")
plsda_data <- cbind( patient_id=train_df  %>% dplyr::pull(Number.x), class=train_df$class, as.data.frame(MyResult.splsda$variates$X))
plsda_data$patient_id <- as.factor(plsda_data$patient_id)
plsda_data %>%
  dplyr::arrange(class) -> plsda_data
plsda_data <- as.data.table(plsda_data)
...
ggsave("../../figures/elisa_plasma_proteomics/elisa_splada_ratio_random.png", plot = splsda_better_ratio_avg_traj_fig, width = 11, height = 8.5, dpi = 200)
ggsave("../../figures/elisa_plasma_proteomics/elisa_splada_ratio_random.svg", plot = splsda_better_ratio_avg_traj_fig, width = 11, height = 8.5, dpi = 200)

```

# Comparison of Using All patients or Subsetting for Unique Patients per Class

Lets compare to see if it makes a difference whether we use all patients for both iGBM/rGBM and rGBM/iGBM classes vs only using a unique set of patients for iGBM/rGBM and rGBM/iGBM (i.e. no duplicate patient in each class)

1. I perform a leave one out cross validation, leaving one patient out and using the rest of the data for training. 
2. The left out patient gets using for assessing model performance accuracy. 
3. The mean accuracy of each left out sample is computed
4. Steps 1-3 is repeated 100 times to assess the variation in mean accuracy of the LOOCV
5. 1-4 is performed for both cases of using all patients and using unique subset patients

```{r echo=FALSE, fig.height=8.5, fig.width=11, message=FALSE, warning=FALSE}
# Get ratio data
ratio_data <- get_ratio_data(data_long2)
ratio_data <- data_wide
colnames(ratio_data) <- make.names(colnames(ratio_data))
ratio_data$class <- factor(ratio_data$class, levels=c("iGBM", "rGBM"))
ratio_data <- ratio_data[Number.x!=32]
# Get unique set of patient ids
patient_ids <- unique(ratio_data$Number.x)

plan(multisession(workers = 8))

# Compute mean accuracy when using unique patients per class for 100 repeats
mean_acc_rep_unique <- future.apply::future_replicate(100, splsda_loocv_perf(ratio_data, patient_ids, unique_random_train_patient_per_class=TRUE))
# hist(mean_acc_rep_unique)
message("Unique Patient Class Split: 95% Confidence Interval:")
print(c(quantile(mean_acc_rep_unique, .025), quantile(mean_acc_rep_unique, .975)))

# Compute mean accuracy when using all patients per class for 100 repeats
mean_acc_rep_all <- future.apply::future_replicate(100, splsda_loocv_perf(ratio_data, patient_ids, unique_random_train_patient_per_class=FALSE))
# hist(mean_acc_rep_all)
message("All Patients per Class: 95% Confidence Interval:")
print(c(quantile(mean_acc_rep_all, .025), quantile(mean_acc_rep_all, .975)))

plan(sequential)

df_plot <- rbind(data.frame(method="Unique Patient Class Split", mean_acc=mean_acc_rep_unique), data.frame(method="All Patients per Class", mean_acc=mean_acc_rep_all))
df_plot$method <- factor(df_plot$method, levels = c("All Patients per Class", "Unique Patient Class Split"))

ggplot(df_plot, aes(x=mean_acc, fill=method)) +
  geom_histogram(binwidth = 0.05, colour = "black") +
  scale_x_continuous(breaks = seq(0, 1, by = 0.2)) +
  scale_fill_brewer(palette = "Dark2") +
  stat_bin(binwidth = 0.05, aes(label=..count..), vjust=-0.5, geom = "text") +
  facet_wrap(~method) +
  labs(x="Mean Accuracy (LOOCV)", y="Count") +
  ggtitle("Mean Accuracy of LOOCV for 100 Repeats") +
  theme_classic() + 
  theme(plot.title = element_text(hjust = 0.5, size=17),
        axis.title = element_text(size = 17),
        axis.text = element_text(size=16),
...
table_theme <- ttheme_minimal(core = list(fg_params=list(col=(RColorBrewer::brewer.pal(5, "Dark2")))), base_size = 13)
...
train_df <- as.data.frame(train_df)
train_df <- median_centre_data(train_df)
train_df <- train_df %>% dplyr::select(class, ASAH1)
train_df$dummy_var <- 0
pred_obj_1.51 <- run_predictions(train_df, pred_col_prob = "rGBM")

## ROC Figure 
g <- ggplot(pred_obj_1.51$roc_data, aes(m=rGBM, d=factor(obs, levels = c("iGBM", "rGBM")), color=Model)) + 
  geom_roc(n.cuts=0) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(length(unique(pred_obj_1.51$master_lift$liftModelVar)), "Dark2"), labels=c("Logit", "Elastic Logit", "KNN", "RF")) +
  coord_equal() +
  style_roc(x="1 - Specificity", y="Sensitivity")

auc_tbl_1.51 <- as.data.table(calc_auc(g))

g <- g + annotate("text", x=0.70, y=0.45, label=paste("AUC =", round(auc_tbl_1.51[Model=="Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.51$master_lift$liftModelVar)), "Dark2")[1])
g <- g + annotate("text", x=0.70, y=0.35, label=paste("AUC =", round(auc_tbl_1.51[Model=="Elastic Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.51$master_lift$liftModelVar)), "Dark2")[2])
g <- g + annotate("text", x=0.70, y=0.25, label=paste("AUC =", round(auc_tbl_1.51[Model=="KNN"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.51$master_lift$liftModelVar)), "Dark2")[3])
g <- g + annotate("text", x=0.70, y=0.15, label=paste("AUC =", round(auc_tbl_1.51[Model=="RF"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.51$master_lift$liftModelVar)), "Dark2")[4])
subfigure_1.51 <- g

pred_obj_1.51$roc_data %>%
  dplyr::rename(ctrl=iGBM, GBM=rGBM) %>%
  dplyr::mutate(pred=plyr::mapvalues(pred, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) %>%
  dplyr::mutate(obs=plyr::mapvalues(obs, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) -> tmp_pred_obj_1.51_roc_data
...
train_df <- as.data.frame(train_df)
train_df <- median_centre_data(train_df)
train_df <- train_df %>% dplyr::select(class, ASAH1, SYNEMIN)
pred_obj_1.52 <- run_predictions(train_df, pred_col_prob = "rGBM")

## ROC Figure 
g <- ggplot(pred_obj_1.52$roc_data, aes(m=rGBM, d=factor(obs, levels = c("iGBM", "rGBM")), color=Model)) + 
  geom_roc(n.cuts=0) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(length(unique(pred_obj_1.52$master_lift$liftModelVar)), "Dark2"), labels=c("Logit", "Elastic Logit", "KNN", "RF")) +
  coord_equal() +
  style_roc(x="1 - Specificity", y="Sensitivity")

auc_tbl_1.52 <- as.data.table(calc_auc(g))

g <- g + annotate("text", x=0.70, y=0.45, label=paste("AUC =", round(auc_tbl_1.52[Model=="Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.52$master_lift$liftModelVar)), "Dark2")[1])
g <- g + annotate("text", x=0.70, y=0.35, label=paste("AUC =", round(auc_tbl_1.52[Model=="Elastic Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.52$master_lift$liftModelVar)), "Dark2")[2])
g <- g + annotate("text", x=0.70, y=0.25, label=paste("AUC =", round(auc_tbl_1.52[Model=="KNN"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.52$master_lift$liftModelVar)), "Dark2")[3])
g <- g + annotate("text", x=0.70, y=0.15, label=paste("AUC =", round(auc_tbl_1.52[Model=="RF"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.52$master_lift$liftModelVar)), "Dark2")[4])
subfigure_1.52 <- g

pred_obj_1.52$roc_data %>%
  dplyr::rename(ctrl=iGBM, GBM=rGBM) %>%
  dplyr::mutate(pred=plyr::mapvalues(pred, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) %>%
  dplyr::mutate(obs=plyr::mapvalues(obs, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) -> tmp_pred_obj_1.52_roc_data
...
train_df <- as.data.frame(train_df)
train_df <- median_centre_data(train_df)
train_df <- train_df %>% dplyr::select(class, ASAH1, SYNEMIN, GPNMB)
pred_obj_1.53 <- run_predictions(train_df, pred_col_prob = "rGBM")

## ROC Figure 
g <- ggplot(pred_obj_1.53$roc_data, aes(m=rGBM, d=factor(obs, levels = c("iGBM", "rGBM")), color=Model)) + 
  geom_roc(n.cuts=0) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(length(unique(pred_obj_1.53$master_lift$liftModelVar)), "Dark2"), labels=c("Logit", "Elastic Logit", "KNN", "RF")) +
  coord_equal() +
  style_roc(x="1 - Specificity", y="Sensitivity")

auc_tbl_1.53 <- as.data.table(calc_auc(g))

g <- g + annotate("text", x=0.70, y=0.45, label=paste("AUC =", round(auc_tbl_1.53[Model=="Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.53$master_lift$liftModelVar)), "Dark2")[1])
g <- g + annotate("text", x=0.70, y=0.35, label=paste("AUC =", round(auc_tbl_1.53[Model=="Elastic Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.53$master_lift$liftModelVar)), "Dark2")[2])
g <- g + annotate("text", x=0.70, y=0.25, label=paste("AUC =", round(auc_tbl_1.53[Model=="KNN"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.53$master_lift$liftModelVar)), "Dark2")[3])
g <- g + annotate("text", x=0.70, y=0.15, label=paste("AUC =", round(auc_tbl_1.53[Model=="RF"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.53$master_lift$liftModelVar)), "Dark2")[4])
subfigure_1.53 <- g

pred_obj_1.53$roc_data %>%
  dplyr::rename(ctrl=iGBM, GBM=rGBM) %>%
  dplyr::mutate(pred=plyr::mapvalues(pred, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) %>%
  dplyr::mutate(obs=plyr::mapvalues(obs, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) -> tmp_pred_obj_1.53_roc_data
...
train_df <- as.data.frame(train_df)
train_df <- median_centre_data(train_df)
train_df <- train_df %>% dplyr::select(class, ASAH1, SYNEMIN, GPNMB, MMP.9)
pred_obj_1.54 <- run_predictions(train_df, pred_col_prob = "rGBM")

## ROC Figure 
g <- ggplot(pred_obj_1.54$roc_data, aes(m=rGBM, d=factor(obs, levels = c("iGBM", "rGBM")), color=Model)) + 
  geom_roc(n.cuts=0) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(length(unique(pred_obj_1.54$master_lift$liftModelVar)), "Dark2"), labels=c("Logit", "Elastic Logit", "KNN", "RF")) +
  coord_equal() +
  style_roc(x="1 - Specificity", y="Sensitivity")

auc_tbl_1.54 <- as.data.table(calc_auc(g))

g <- g + annotate("text", x=0.70, y=0.45, label=paste("AUC =", round(auc_tbl_1.54[Model=="Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.54$master_lift$liftModelVar)), "Dark2")[1])
g <- g + annotate("text", x=0.70, y=0.35, label=paste("AUC =", round(auc_tbl_1.54[Model=="Elastic Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.54$master_lift$liftModelVar)), "Dark2")[2])
g <- g + annotate("text", x=0.70, y=0.25, label=paste("AUC =", round(auc_tbl_1.54[Model=="KNN"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.54$master_lift$liftModelVar)), "Dark2")[3])
g <- g + annotate("text", x=0.70, y=0.15, label=paste("AUC =", round(auc_tbl_1.54[Model=="RF"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.54$master_lift$liftModelVar)), "Dark2")[4])
subfigure_1.54 <- g

pred_obj_1.54$roc_data %>%
  dplyr::rename(ctrl=iGBM, GBM=rGBM) %>%
  dplyr::mutate(pred=plyr::mapvalues(pred, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) %>%
  dplyr::mutate(obs=plyr::mapvalues(obs, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) -> tmp_pred_obj_1.54_roc_data

master_Table <- rbindlist(list(`1 Protein`=tmp_pred_obj_1.51_roc_data, `1-2 Proteins`=tmp_pred_obj_1.52_roc_data, `1-3 Proteins`=tmp_pred_obj_1.53_roc_data, `1-4 Proteins`=tmp_pred_obj_1.54_roc_data), idcol = 'Number of Proteins')


master_auc_data <- rbindlist(list(`1 Protein`=auc_tbl_1.51, `1-2 Proteins`=auc_tbl_1.52, `1-3 Proteins`=auc_tbl_1.53, `1-4 Proteins`=auc_tbl_1.54), idcol = "Number of Proteins")


master_auc_data[Model %in% c("Elastic Logit")] %>%
  dplyr::mutate(AUC = sprintf("AUC: %s%%", round(AUC*100, 0))) %>%
  tidyr::pivot_wider(id_cols = "Number of Proteins", names_from = Model, values_from = AUC) -> auc_annotation_tbl

table_theme <- ttheme_minimal(core = list(fg_params=list(col=(RColorBrewer::brewer.pal(5, "Dark2")))), base_size = 13)
auc_grob <- tableGrob(auc_annotation_tbl, rows=NULL, theme = table_theme)

g <- ggplot(master_Table[Model %in% c("Elastic Logit")], aes(m=GBM, d=factor(obs, levels = c("ctrl", "GBM")), color=`Number of Proteins`), linetype='solid') +
  geom_roc(n.cuts=0, size=1.5) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(5, "Dark2")) +
  # scale_linetype_manual(values=c("solid", "dotted"), labels=c("Elastic Logit")) + 
  coord_equal() +
...
## 1 Protein ----
train_df_sub <- train_df %>% dplyr::select(class, ASAH1)
train_df_sub$dummy_var <- 0
pred_obj_1.31 <- run_predictions(train_df_sub, pred_col_prob = "rGBM")

## ROC Figure 
g <- ggplot(pred_obj_1.31$roc_data, aes(m=rGBM, d=factor(obs, levels = c("iGBM", "rGBM")), color=Model)) +
  geom_roc(n.cuts=0) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(length(unique(pred_obj_1.31$master_lift$liftModelVar)), "Dark2"), labels=c("Logit", "Elastic Logit", "KNN", "RF")) +
  coord_equal() +
  style_roc(x="1 - Specificity", y="Sensitivity")

auc_tbl_1.31 <- as.data.table(calc_auc(g))

g <- g + annotate("text", x=0.70, y=0.45, label=paste("AUC =", round(auc_tbl_1.31[Model=="Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.31$master_lift$liftModelVar)), "Dark2")[1])
g <- g + annotate("text", x=0.70, y=0.35, label=paste("AUC =", round(auc_tbl_1.31[Model=="Elastic Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.31$master_lift$liftModelVar)), "Dark2")[2])
g <- g + annotate("text", x=0.70, y=0.25, label=paste("AUC =", round(auc_tbl_1.31[Model=="KNN"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.31$master_lift$liftModelVar)), "Dark2")[3])
g <- g + annotate("text", x=0.70, y=0.15, label=paste("AUC =", round(auc_tbl_1.31[Model=="RF"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.31$master_lift$liftModelVar)), "Dark2")[4])
subfigure_1.31 <- g

pred_obj_1.31$roc_data %>%
  dplyr::rename(ctrl=iGBM, GBM=rGBM) %>%
  dplyr::mutate(pred=plyr::mapvalues(pred, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) %>%
  dplyr::mutate(obs=plyr::mapvalues(obs, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) -> tmp_pred_obj_1.31_roc_data

## 2 Protein ----
train_df_sub <- train_df %>% dplyr::select(class, ASAH1, SYNEMIN)
pred_obj_1.32 <- run_predictions(train_df_sub, pred_col_prob = "rGBM")

## ROC Figure 
g <- ggplot(pred_obj_1.32$roc_data, aes(m=rGBM, d=factor(obs, levels = c("iGBM", "rGBM")), color=Model)) +
  geom_roc(n.cuts=0) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(length(unique(pred_obj_1.32$master_lift$liftModelVar)), "Dark2"), labels=c("Logit", "Elastic Logit", "KNN", "RF")) +
  coord_equal() +
  style_roc(x="1 - Specificity", y="Sensitivity")

auc_tbl_1.32 <- as.data.table(calc_auc(g))

g <- g + annotate("text", x=0.70, y=0.45, label=paste("AUC =", round(auc_tbl_1.32[Model=="Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.32$master_lift$liftModelVar)), "Dark2")[1])
g <- g + annotate("text", x=0.70, y=0.35, label=paste("AUC =", round(auc_tbl_1.32[Model=="Elastic Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.32$master_lift$liftModelVar)), "Dark2")[2])
g <- g + annotate("text", x=0.70, y=0.25, label=paste("AUC =", round(auc_tbl_1.32[Model=="KNN"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.32$master_lift$liftModelVar)), "Dark2")[3])
g <- g + annotate("text", x=0.70, y=0.15, label=paste("AUC =", round(auc_tbl_1.32[Model=="RF"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.32$master_lift$liftModelVar)), "Dark2")[4])
subfigure_1.32 <- g

pred_obj_1.32$roc_data %>%
  dplyr::rename(ctrl=iGBM, GBM=rGBM) %>%
  dplyr::mutate(pred=plyr::mapvalues(pred, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) %>%
  dplyr::mutate(obs=plyr::mapvalues(obs, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) -> tmp_pred_obj_1.32_roc_data

## 3 Protein ----
train_df_sub <- train_df %>% dplyr::select(class, ASAH1, SYNEMIN, GPNMB)
pred_obj_1.33 <- run_predictions(train_df_sub, pred_col_prob = "rGBM")

## ROC Figure 
g <- ggplot(pred_obj_1.33$roc_data, aes(m=rGBM, d=factor(obs, levels = c("iGBM", "rGBM")), color=Model)) +
  geom_roc(n.cuts=0) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(length(unique(pred_obj_1.33$master_lift$liftModelVar)), "Dark2"), labels=c("Logit", "Elastic Logit", "KNN", "RF")) +
  coord_equal() +
  style_roc(x="1 - Specificity", y="Sensitivity")

auc_tbl_1.33 <- as.data.table(calc_auc(g))

g <- g + annotate("text", x=0.70, y=0.45, label=paste("AUC =", round(auc_tbl_1.33[Model=="Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.33$master_lift$liftModelVar)), "Dark2")[1])
g <- g + annotate("text", x=0.70, y=0.35, label=paste("AUC =", round(auc_tbl_1.33[Model=="Elastic Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.33$master_lift$liftModelVar)), "Dark2")[2])
g <- g + annotate("text", x=0.70, y=0.25, label=paste("AUC =", round(auc_tbl_1.33[Model=="KNN"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.33$master_lift$liftModelVar)), "Dark2")[3])
g <- g + annotate("text", x=0.70, y=0.15, label=paste("AUC =", round(auc_tbl_1.33[Model=="RF"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.33$master_lift$liftModelVar)), "Dark2")[4])
subfigure_1.33 <- g

pred_obj_1.33$roc_data %>%
  dplyr::rename(ctrl=iGBM, GBM=rGBM) %>%
  dplyr::mutate(pred=plyr::mapvalues(pred, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) %>%
  dplyr::mutate(obs=plyr::mapvalues(obs, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) -> tmp_pred_obj_1.33_roc_data

## 4 Protein ----
train_df_sub <- train_df %>% dplyr::select(class, ASAH1, SYNEMIN, GPNMB, MMP.9)
pred_obj_1.34 <- run_predictions(train_df_sub, pred_col_prob = "rGBM")

## ROC Figure 
g <- ggplot(pred_obj_1.34$roc_data, aes(m=rGBM, d=factor(obs, levels = c("iGBM", "rGBM")), color=Model)) +
  geom_roc(n.cuts=0) + 
  scale_color_manual(values = RColorBrewer::brewer.pal(length(unique(pred_obj_1.34$master_lift$liftModelVar)), "Dark2"), labels=c("Logit", "Elastic Logit", "KNN", "RF")) +
  coord_equal() +
  style_roc(x="1 - Specificity", y="Sensitivity")

auc_tbl_1.34 <- as.data.table(calc_auc(g))

g <- g + annotate("text", x=0.70, y=0.45, label=paste("AUC =", round(auc_tbl_1.34[Model=="Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.34$master_lift$liftModelVar)), "Dark2")[1])
g <- g + annotate("text", x=0.70, y=0.35, label=paste("AUC =", round(auc_tbl_1.34[Model=="Elastic Logit"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.34$master_lift$liftModelVar)), "Dark2")[2])
g <- g + annotate("text", x=0.70, y=0.25, label=paste("AUC =", round(auc_tbl_1.34[Model=="KNN"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.34$master_lift$liftModelVar)), "Dark2")[3])
g <- g + annotate("text", x=0.70, y=0.15, label=paste("AUC =", round(auc_tbl_1.34[Model=="RF"]$AUC, 4)), color=RColorBrewer::brewer.pal(length(unique(pred_obj_1.34$master_lift$liftModelVar)), "Dark2")[4])
subfigure_1.34 <- g

pred_obj_1.34$roc_data %>%
  dplyr::rename(ctrl=iGBM, GBM=rGBM) %>%
  dplyr::mutate(pred=plyr::mapvalues(pred, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) %>%
  dplyr::mutate(obs=plyr::mapvalues(obs, from=c("iGBM", "rGBM"), to=c("ctrl", "GBM"))) -> tmp_pred_obj_1.34_roc_data

## Merged Plot
master_Table <- rbindlist(list(`1 Protein`=tmp_pred_obj_1.31_roc_data, `1-2 Proteins`=tmp_pred_obj_1.32_roc_data, `1-3 Proteins`=tmp_pred_obj_1.33_roc_data, `1-4 Proteins`=tmp_pred_obj_1.34_roc_data), idcol = 'Number of Proteins')


master_auc_data <- rbindlist(list(`1 Protein`=auc_tbl_1.31, `1-2 Proteins`=auc_tbl_1.32, `1-3 Proteins`=auc_tbl_1.33, `1-4 Proteins`=auc_tbl_1.34), idcol = "Number of Proteins")
...
<!-- # Latency Data -->
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.width=9, include=FALSE}
latency_data_file <- here::here("data/elisa_plasma_proteomics/ELISA_LATENCY_KM.csv")
...
survival_plot_grid <- ggarrange(surv_p1, surv_p2, surv_p3, surv_p4, ncol = 4)
...
ggsave("../../figures/elisa_plasma_proteomics/elisa_main_subfigure_c1.png", plot = surv_p1, width = 11, height = 7, dpi = 200)
ggsave("../../figures/elisa_plasma_proteomics/elisa_main_subfigure_c1.svg", plot = surv_p1, width = 11, height = 7, dpi = 200)

ggsave("../../figures/elisa_plasma_proteomics/elisa_main_subfigure_c2.png", plot = surv_p2, width = 11, height = 7, dpi = 200)
ggsave("../../figures/elisa_plasma_proteomics/elisa_main_subfigure_c2.svg", plot = surv_p2, width = 11, height = 7, dpi = 200)

ggsave("../../figures/elisa_plasma_proteomics/elisa_main_subfigure_c3.png", plot = surv_p3, width = 11, height = 7, dpi = 200)
ggsave("../../figures/elisa_plasma_proteomics/elisa_main_subfigure_c3.svg", plot = surv_p3, width = 11, height = 7, dpi = 200)

ggsave("../../figures/elisa_plasma_proteomics/elisa_main_subfigure_c4.png", plot = surv_p4, width = 11, height = 7, dpi = 200)
ggsave("../../figures/elisa_plasma_proteomics/elisa_main_subfigure_c4.svg", plot = surv_p4, width = 11, height = 7, dpi = 200)
...
library(readr)
write_tsv(ttr_surv_data,
          here::here("data/elisa_plasma_proteomics/ttr_survival_data.tsv"))
...
## All 4 ----
fit <- survfit(Surv(Time.to.reccurence..days., status) ~ ASAH1 + SYNM + GPNMB + MMP9, data = ttr_surv_data)
print(fit)
# Change color, linetype by strata, risk.table color by strata
ttr_surv_p5 <- ggsurvplot(fit,
          pval = TRUE, pval.coord = c(650, 0.85), conf.int = F,
          ...
          legend.title = "",
          legend.labs = c(
            sprintf("ASAH1=high, SYNM=high, GPNMB=high, MMP9=high (n=1)"), 
            sprintf("ASAH1=high, SYNM=low, GPNMB=high, MMP9=low (n=1)"),  
            sprintf("ASAH1=low, SYNM=high, GPNMB=low , MMP9=high (n=2)"), 
            sprintf("ASAH1=high, SYNM=high, GPNMB=low , MMP9=low (n=1)"),
            sprintf("ASAH1=high, SYNM=low , GPNMB=high, MMP9=low (n=1)"),
            sprintf("ASAH1=high, SYNM=low , GPNMB=low , MMP9=high (n=1)"),
            sprintf("ASAH1=high, SYNM=low , GPNMB=low , MMP9=low (n=2)"),
            sprintf("ASAH1=low, SYNM=high, GPNMB=high, MMP9=high (n=1)"),
            sprintf("ASAH1=low, SYNM=high, GPNMB=high, MMP9=low (n=1)"),
            sprintf("ASAH1=low, SYNM=high, GPNMB=low , MMP9=high (n=2)"),
            sprintf("ASAH1=low, SYNM=low , GPNMB=high, MMP9=low (n=3)"),
            sprintf("ASAH1=low, SYNM=low , GPNMB=low , MMP9=high (n=1)"),
            sprintf("ASAH1=low, SYNM=low , GPNMB=low , MMP9=low (n=2)")
            )
          ) +
  labs(x="Time to Recurrence (Days)", y="Time to Recurrence probability")  +
  guides(col=guide_legend(nrow=4,byrow=TRUE))
...
ttr_surv_data %>%
  surv_plot(dependent_var, explanatory_vars, xlab="Time (days)", pval=TRUE, legend="none")


ttr_surv_data %>%
  ff_plot(dependent_var, explanatory_vars, dependent_label="Time to Recurrence")
```
...
**Figure 3: A Promising Plasma Proteomics Signature for Stratification and Prediction of Recurrent Glioblastoma Multiforme.** Concentration of four proteins identified from TMT-MS tissue proteomics, were measured in patients' plasma samples using Enzyme-Linked Immunosorbent Assay (ELISA). (**a**) The log2 transformed protein concentration measurements demonstrates the difference in log2 concentration distributions between control, initial GBM and paired recurrent GBM populations. Note, a patient was omitted due to being an outlier, see Supp Figure X for full set of patients. (**b**)  Kaplan-Meyer plots showing demonstrating high concentration of ASAH1 and high concentration of SYNEMIN contributing to a higher probability of tumor recurrence occurring at a later date. (**c**) The receiver operating characteristic (ROC) shows the potential of using plasma proteomics for separating each population, driven mostly from information contained in the proteins. (**d**) Reducing the data of the four proteins ratios demonstrates an average trajectory and separation of $\frac{initial \quad GBM}{recurrent \quad GBM}$ vs $\frac{recurrent \quad GBM}{initial \quad GBM}$. One patient was randomly left out as a test patient (black + point), 4 samples were randomly sampled to represent $\frac{initial \quad GBM}{recurrent \quad GBM}$ (blue points) and remaining 4 samples were used to represent $\frac{recurrent \quad GBM}{initial \quad GBM}$.
...
ttr_survival_plot_grid <- ggarrange(surv_p1, surv_p2, surv_p3, surv_p4, ncol = 4, labels="A")
bottom_plots <- ggarrange(ggarrange(ttr_surv_p3, ttr_surv_p4, ncol=2), ggarrange(surv_p5, ttr_surv_p5, ncol=2), labels=c("B", "C"))

supp_figure_4 <- ggarrange(ttr_survival_plot_grid, bottom_plots, nrow=2, ncol=1)
supp_figure_4

ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s.png", fig_counter), plot = supp_figure_4, width = 25, height = 15, dpi = 200)
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s.svg", fig_counter), plot = supp_figure_4, width = 25, height = 15, dpi = 200)
...
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_a_1.png", fig_counter), plot = ttr_surv_p1, width = 11, height = 8.5, dpi = 200)
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_a_1.svg", fig_counter), plot = ttr_surv_p1, width = 11, height = 8.5, dpi = 200)

ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_a_2.png", fig_counter), plot = ttr_surv_p2, width = 11, height = 8.5, dpi = 200)
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_a_2.svg", fig_counter), plot = ttr_surv_p2, width = 11, height = 8.5, dpi = 200)

ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_a_3.png", fig_counter), plot = ttr_surv_p3, width = 11, height = 8.5, dpi = 200)
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_a_3.svg", fig_counter), plot = ttr_surv_p3, width = 11, height = 8.5, dpi = 200)

ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_a_4.png", fig_counter), plot = ttr_surv_p4, width = 11, height = 8.5, dpi = 200)
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_a_4.svg", fig_counter), plot = ttr_surv_p4, width = 11, height = 8.5, dpi = 200)

ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_b.png", fig_counter), plot = surv_p5, width = 9, height = 8, dpi = 200)
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_b.svg", fig_counter), plot = surv_p5, width = 9, height = 8, dpi = 200)

ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_c.png", fig_counter), plot = ttr_surv_p5, width = 9, height = 8, dpi = 200)
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_c.svg", fig_counter), plot = ttr_surv_p5, width = 9, height = 8, dpi = 200)

ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_c_with_ci.png", fig_counter), plot = ttr_surv_p5, width = 9, height = 8, dpi = 200)
ggsave(sprintf("../../figures/elisa_plasma_proteomics/supp_figure_%s_subfigure_c_with_ci.svg", fig_counter), plot = ttr_surv_p5, width = 9, height = 8, dpi = 200)
```
